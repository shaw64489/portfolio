...
//toggle mark icon state on click
function onClickMark(event, type, editor) {
  event.preventDefault();
  editor.toggleMark(type);
}

//render hover icon option
...
const onClickBlock = (event, type, editor) => {
  ...
  if (type != 'bulleted-list' && type != 'numbered-list') {
    ...
    if (isList) {
      ...
      //if not list set type
      editor.setBlocks(isActive ? DEFAULT_NODE : type);
    }
  //if type is list  
  } else {
    // Handle the extra wrapping required for list buttons.
    //is selected item a list item
    const isList = hasBlock('list-item', value);
    //check if parent is of same type
    const isType = value.blocks.some(block => {
      return !!document.getClosest(block.key, parent => parent.type == type);
    });

    //list aand parent match list type  - unwrap
    if (isList && isType) {
      editor
